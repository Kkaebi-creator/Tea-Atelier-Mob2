...
export async function GET(req: Request) {
  ...
  return addCorsHeaders(NextResponse.json({ addresses: rows }));
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!addressLine1?.trim()) {
      return addCorsHeaders(NextResponse.json({ error: "Address line 1 is required." }, { status: 400 }));
    }
    ...
    return addCorsHeaders(NextResponse.json({ address: result.rows[0] }));
  } catch (error) {
    console.error("Create address error:", error);
    return addCorsHeaders(NextResponse.json({ error: "Unable to save address." }, { status: 500 }));
  }
}